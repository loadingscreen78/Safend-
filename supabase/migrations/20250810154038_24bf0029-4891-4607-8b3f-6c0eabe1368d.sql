-- 1) Enum for roles (safe create)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'app_role') THEN
    CREATE TYPE public.app_role AS ENUM ('admin','sales','operations','accounts','hr','reports');
  END IF;
END$$;
...
SET search_path = ''
AS $$
  SELECT role FROM public.user_roles WHERE user_id = COALESCE(_user_id, auth.uid());
$$;
...
CREATE OR REPLACE FUNCTION public.assign_role(_user_id uuid, _email text, _role public.app_role)
...
-- 7) RLS policies for user_roles (safe drop/create)
DO $$ BEGIN
